...
// 计算点光
vec3 calcPointLight(Material material, PointLight pointLight, vec3 normal, vec3 eyeDir, vec3 fragPos) {
...
void main() {
    ...
    vec3 ambient = texture(gAmbientShininess, texcoord).xyz;
    float shininess = texture(gAmbientShininess, texcoord).w;
    Material material = Material(ambient, diffuse, specular, shininess);
    ...
    for(int j = 0; j < NR_POINT_LIGHTS; j++) {
        result += calcPointLight(material, pointLights[j], norm, eyeDir, fragPos);
    }
